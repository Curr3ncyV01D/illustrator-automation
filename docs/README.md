...
- **Мост (Bridge)**: Python-сервис (FastAPI) для управления Adobe Illustrator через COM-интерфейс.
...
[ User (Telegram) ] 
        ↓
[ Logical Core (Docker/Linux) ]  <-- n8n оркестрация и управление очередью
        ↓
[ Bridge ]     <-- Python-мост на Windows машине
        ↓
[ Graphical Core ]    <-- Adobe Illustrator (ExtendScript)
```
